	...
	<body>
			...
			<div class="loginCnt">
				...
				<div class="login-box">
					<ul>
						...
						<li>
						<input type="email" id="email" name="email" class="login-text input" placeholder="请输入邮箱号"><input type="button" id="bu" value="点击发送">
						</li>
						<li><input type="text" id="code" class="login-text input" placeholder="请输入验证码"></li>
						<li class="login-t">
							<span>
		...
		/* $(function(){ */
			$("#bu").click(function(){
				...
				$.ajax({
					type:"post",
					url:"${pageContext.request.contextPath}/user/fa.do",
					data:{
						email:email
					},
		...
		$(function(){
			...
			$('#sbtn').click(function(){
				...
				/* $.post("${pageContext.request.contextPath }/regist.do",$("#formId").serialize(),function(data){
					if(data.status==200){
						alert(data.msg);
				...
				if(tosub){
			  		$.ajax({
			  			...
			  			type:"POST",
			  			dataType:"text",
			  			data:{"name":$("#phone_ipt").val(),"code":$("#code").val()},
			  			success:function(data){
			  				if(data=="field"){
